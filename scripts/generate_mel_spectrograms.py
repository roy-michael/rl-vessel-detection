...
def generate_mel(dataset_name, file_dir):
    print(f"\nProcessing Mel spectrogram for {dataset_name}...")
    ...
    # Standard Mel Spectrogram (No background normalization)
    plot_data = spec_db
    ...
    ax.set_title(f"Standard Mel Spectrogram — {title_ds}\n(Timeframe: {total_duration/60.0:.1f} mins | Unnormalized Power{start_time_str})", 
    ...
    cbar.set_label("Signal Power (dB)", fontsize=10, fontweight="bold")
    cbar.ax.tick_params(labelsize=9)
    
    out_dir = os.path.join("output", dataset_name)
    os.makedirs(out_dir, exist_ok=True)
    output_filename = f"mel_spectrogram_{dataset_name}.png"
    output_path = os.path.join(out_dir, output_filename)
    fig.savefig(output_path, bbox_inches="tight", dpi=180, facecolor=DARK_BG)
    plt.close(fig)
    print(f"Saved Standard Mel spectrogram to: {output_path}")
