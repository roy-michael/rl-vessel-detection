...
def main():
    ...
    if not args.skip_training:
        ...
        for aid, label in zip(agent_ids, agent_labels):
            ...
            if rewards:
                x_vals = range(1, len(rewards) + 1)
                ax_comb.plot(x_vals, rewards, marker='o', markersize=2, linewidth=0.5, alpha=0.15, color=COLORS[aid])
                if len(rewards) > 5:
                    trend = gaussian_filter1d(rewards, sigma=5.0)
                    ax_comb.plot(x_vals, trend, linewidth=4.0, color=COLORS[aid], label=label.replace("\n", " "), zorder=10)
                else:
                    ax_comb.plot(x_vals, rewards, linewidth=4.0, color=COLORS[aid], label=label.replace("\n", " "), zorder=10)
        ...
        for idx, (aid, label) in enumerate(zip(agent_ids, agent_labels)):
            ...
            if rewards:
                x_vals = range(1, len(rewards) + 1)
                ax.plot(x_vals, rewards, marker='o', markersize=2, linewidth=0.5, alpha=0.15, color=COLORS[aid])
                if len(rewards) > 5:
                    trend = gaussian_filter1d(rewards, sigma=5.0)
                    ax.plot(x_vals, trend, linewidth=4.0, color=COLORS[aid], zorder=10)
                else:
                    ax.plot(x_vals, rewards, linewidth=4.0, color=COLORS[aid], zorder=10)
