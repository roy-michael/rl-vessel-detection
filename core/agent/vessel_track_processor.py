...
class VesselTrackProcessor:
    """
    A signal tracking component managed by the DSPOrchestrator.
    Represents and manages the tracking lifecycle of a single physical vessel.
    Maintains a continuous history of spectral observations derived from the orchestrator.
    """
    def __init__(self, env, start_time, initial_freq, initial_spread, initial_amp, vessel_id):
        self.env = env  # The DSPOrchestrator providing the data stream
