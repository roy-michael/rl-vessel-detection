...
3. To evaluate and compare the RL algorithm families under rigorous training (500 episodes on the `Croatia 2307` dataset) to demonstrate stable policy convergence.
...
1.  **Croatia Ocean Sonics Dataset (Silba Island)**: Comprising recordings from high-frequency hydrophones deployed near Zapadni Greben, Silba Island, Croatia. These recordings capture the acoustic signature of Suex VRX Diver Propulsion Vehicles (DPVs) along with standard surface vessels (e.g., small motorboats and Ro-Ro passenger ferries) to create realistic, overlapping interference scenarios.
2.  **Haifa Dataset**: Focusing on the acoustic signature of a Seacraft GO! Diver Propulsion Vehicle (underwater scooter) maneuvering on prescribed circular/linear tracks in shallow coastal waters (18-25 m depth) in Haifa, Israel.

#### Recording Equipment & Setup
Acoustic measurements were recorded using high-performance equipment to ensure fidelity:
*   **Acoustic Recorder**: An `icListen HF ALTA` acoustic recorder unit, sampling continuous data at **128 kHz** (and 96 kHz for select Haifa trials), saving data in lossless `.wav` format.
*   **Hydrophones**: A preamplified `Colmar GP 0280` hydrophone connected via a 100m low-noise cable, alongside laboratory-anchored mooring hydrophone frames. 
*   **Mooring Configuration**: For stationary trials, hydrophones were attached to seabed-anchored mooring lines at depths of approximately 10–30 meters (matching the divers' dive depths) to eliminate surface wave action and buoy movement noise.

A comprehensive summary of the conducted experiments is detailed in the table below:

| Date | Location | Duration (min) | Max Distance (m) | Speed (m/s) | Route Type | Vehicle Used | Recorder |
| :--- | :--- | :---: | :---: | :---: | :--- | :--- | :--- |
| **12/06/2025** | Haifa, Israel | 31 | 500 | 0.7 | Circular | Seacraft GO! DPV | icListen 128kHz |
| **23/07/2025** | Silba, Croatia | 50 | 50 | 2.5 | Linear | Suex VRX DPV | icListen 128kHz & Colmar 44.1kHz |
| **24/07/2025** | Silba, Croatia | 34 | 600 | 0.7, 2.2 | Linear | Suex VRX DPV (with Ferry noise) | icListen 128kHz |
| **24/07/2025** | Silba, Croatia | 37 | 500 | 2.2 | Serpentine | Suex VRX DPV | icListen 128kHz |
| **25/07/2025** | Silba, Croatia | 25 | 1000 | 0.7, 2.2 | Linear | Suex VRX DPV | icListen 128kHz |
| **25/07/2025** | Silba, Croatia | 16 | 100 | 2.2 | Circular | Suex VRX + Support Boat | icListen 128kHz |
| **05-08/08/2025** | Haifa, Israel | 54,000 | — | — | Control | Mid-sea shipping noise control | icListen 128kHz |
| **12/08/2025** | Haifa, Israel | 700 | — | — | Control | Haifa port overlapping ship noise | icListen 96kHz |
...
*Figure 1.1: Calibrated hydrophone mooring platform deployment setup on the seabed (Silba 1k).*
...
In this reinforcement learning formulation, there is a clear distinction between the agent's **Policy (<i>π</i>)**, the **Actions (<i>a</i> ∈ <i>A</i>)**, and the **System Dynamics** (both the physical target motion and the environment's state transition probability <i>P</i>(<i>s'</i> | <i>s</i>, <i>a</i>)):
...
*   **`tracking_mdp_env.py`**: Acts as the standard MDP interface. It observes the acoustic data, calculates the continuous and discrete **State (<i>S</i>)** representations, executes the **Actions (<i>A</i>)** requested by the agent, and calculates the resulting **Reward (<i>R</i>)** via the isolated `TrackingRewardCalculator`.
...
*Figure 3.2: Detailed component interactions within the hybrid RL-DSP tracking framework.*
...
All agents were trained for 500 episodes on the `Croatia 2307` dataset. Below is the quantitative results and analysis.
...
![RL Evaluation Metrics Comparison](./images/rl_comparison_croatia_2307.png?v=2)
*Figure 5.1: Comparative tracking metrics for the Double Q-learning and Linear FA agents on the Croatia 2307 dataset.*
...
![Training Convergence Combined Plot](./images/convergence_combined.png?v=2)
*Figure 5.2: Comparative Policy Convergence over 500 training episodes showing the smoothed rolling average returns.*

![Training Convergence Individual Plot Grid](./images/convergence_individual.png?v=2)
...
![Vessel Timeline Chart](./images/croatia_2307_double_q_learning_timeline.png)
*Figure 5.4: Reconstructed Vessel Timeline (Double Q-Learning) mapped to absolute real-world time.*
...
Our hybrid Reinforcement Learning and NMF framework addresses these limitations in two major ways:
...
We have successfully designed, implemented, and evaluated a reinforcement learning vessel tracking system. Decoupling the MDP variables (State, Action, and Reward Calculator) allowed us to benchmark multiple RL algorithms under a unified interface. Our Double Q-Learning agent successfully converges within 500 episodes on the `Croatia 2307` dataset. The absolute time integration maps tracking timelines directly to real-world clock times (HH:MM:SS), confirming that reinforcement learning offers a highly viable, robust alternative to heuristic tracker architectures.
...
### 8.5 Batch Evaluation Procedure: All Datasets, All Agents (using `croatia_2407_1` policy)
...
### 8.6 Running Automated Verification Suite (Self-Training Loop)
...
Spectrograms contain highly overlapping signals (multiple vessels operating simultaneously) masked by ambient ocean noise. The comparison below displays the raw spectrogram alongside the annotated version marking key tonality elements:

<table style="width: 100%; border: none; table-layout: fixed;">
  <tr style="border: none;">
    <td style="width: 65%; text-align: center; border: none; padding: 5px; vertical-align: top;">
      <img src="./images/LOFAR_Joint_Signal.png" style="height: 350px; max-width: 100%; object-fit: contain;" />
      <br>
      <em>Figure A.1a: Raw LOFAR spectrogram showing overlapping vessel target signatures and ambient noise.</em>
    </td>
    <td style="width: 35%; text-align: center; border: none; padding: 5px; vertical-align: top;">
      <img src="./images/lofar_annotated.png?v=2" style="height: 350px; max-width: 100%; object-fit: contain;" />
      <br>
      <em>Figure A.1b: Annotated LOFAR spectrogram showing base frequencies (green) and harmonics (orange/red).</em>
    </td>
  </tr>
</table>
